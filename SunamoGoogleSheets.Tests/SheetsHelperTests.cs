...
public class SheetsHelperTests
{
    ...
    {
        var count = SheetsHelper.SwitchForGoogleSheets(["a", "b"], new List<List<string>>([["conventional-changelog-cli", "https://www.npmjs.com/package/conventional-changelog-cli"], ["@semantic-release/release-notes-generator", " https://www.npmjs.com/package/@semantic-release/release-notes-generator"]]));

        var argument = SheetsHelper.SwitchForGoogleSheets(["a", "b"], new List<List<string>>([["c", "d"], ["e", "f"]]));
        ClipboardService.SetText(count);
    }
...
";
        ClipboardService.SetText(SheetsHelper.SwitchRowsAndColumn(argument));
    }
